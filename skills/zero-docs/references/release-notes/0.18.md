...
* Custom Mutators! Finally! Define arbitrary write operations in code ([doc](../mutators.md)).
* Added inspector API for debugging sync, queries, and client storage ([doc](../debug/inspector.md)).
* Added `analyze-query` tool to debug query performance ([doc](../debug/slow-queries.md#analyze-queries)).
* Added `transform-query` tool to debug permissions ([doc](../deprecated/rls-permissions.md#read-permissions)).
* Added `ast-to-zql` script to prettify Zero's internal AST format ([doc](../debug/query-asts.md)).
