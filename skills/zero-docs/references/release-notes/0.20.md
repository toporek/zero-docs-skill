...
* Supabase is now fully supported. After upgrading, you should see that schema changes are incremental and don't reset the replica ([docs](../connecting-to-postgres.md)).
...
* Autodiscovery for `replication-manager`. `view-syncer` nodes now find `replication-manager` using the Postgres `changedb` database, and no longer need an internal load balancer. See the new `ZERO_CHANGE_STREAMER_MODE: "discover"` config in the [deployment docs](../self-host.md) ([PR](https://github.com/rocicorp/mono/pull/4335)).
...
* OpenTelemetry support ([docs](../otel.md)).
