...
[Zero 0.15 release notes](https://zero.rocicorp.dev/video/releases/be-sweet-0.15-v1.mp4)
...
3. Use [`npx zero-deploy-permissions`](../deprecated/rls-permissions.md#permission-deployment) to deploy permissions when necessary. You can hook this up to your CI to automate it. See the [zbugs implementation](https://github.com/rocicorp/mono/blob/86ab73122a0532e4ec516badc1d8fb82b3465b49/prod/sst/sst.config.ts#L178) as an example.

## Features

* Live-updating permissions ([docs](../deprecated/rls-permissions.md#permission-deployment)).
* Permissions now default to **deny** rather than **allow** ([docs](../deprecated/rls-permissions.md#access-is-denied-by-default)).
...
* Making permissions deny by default breaks existing apps. To fix add `ANYONE_CAN` or other appropriate permissions for your tables. See [docs](../deprecated/rls-permissions.md#access-is-denied-by-default).
* The `ZERO_SCHEMA_JSON` and `ZERO_SCHEMA_FILE` environment variables are no longer used. Remove them from your setup and use [`npx zero-deploy-permissions`](../deprecated/rls-permissions.md#permission-deployment) instead.
