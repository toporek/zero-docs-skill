...
* **Read permissions.** You can now control read access to data using ZQL ([docs](../deprecated/rls-permissions.md#select-permissions)).
* **Deployment.** We now have a single-node Docker container ([docs](../self-host.md)). Future work will add multinode support.
* **Compound FKs.** Zero already supported compound *primary* keys, but now it also supports compound *foreign* keys ([docs](../schema.md#relationships)).
* **Schema DX**:
