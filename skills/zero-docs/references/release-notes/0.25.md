...
Conceptually, Zero is now just [schemas](../schema.md), [queries](../queries.md), and [mutators](../mutators.md).
...
* [**Query Planning:**](../zql.md#planning) Zero now automatically plans joins, similar to other databases. You no longer need to specify `flip:true` to optimize `whereExists()` calls.
* [**Reworked Query DX:**](../queries.md) Queries now receive a `Context` automatically – you don't have to pass it at the call site. Also, any [Standard Schema](https://standardschema.dev/) conforming library can be used for validation and the API has been generally simplified and streamlined.
* [**Reworked Mutator DX:**](../mutators.md) The mutator API has been overhauled to match queries. Mutators now also support validation.
* [**drizzle-zero and prisma-zero are now first-class:**](../schema.md#generating-from-database) They have moved into the rocicorp org and are now officially supported.
* [**Default Schema Type:**](../schema.md#register-schema-type) The `Schema` type can now be registered as a default with Zero. You can just say `useZero()` – `useZero<Schema>` is no longer needed. You can also delete your `createUseZero()` helper if you have one. This registration happens by default if you use the new `drizzle-zero` or `prisma-zero` packages.
* [**New Connection Status API:**](../connection.md) Brand new connection status API with overhauled error-handling and reconnection behavior.
* [**Mutator promises now return structured errors:**](../mutators.md#waiting-for-results) The `client` and `server` promises now always resolve. In the case of an error, they now return a structured error object.
* [**New Install Guide:**](../install.md) Rewritten guide to add Zero to an existing project.
* [**New zero-out tool:**](../debug/zero-out.md) Removes all trace of Zero from Postgres.
* [**Support for Generated Columns:**](../postgres-support.md#object-types) In Postgres 18+, `generated stored` columns are now synced. This is useful for denormalizing data from JSON columns to support filtering in Zero.
...
1. If you do not want to immediately opt-in to the new query and mutator APIs, set [`enableLegacyQueries`](../deprecated/ad-hoc-queries.md) and [`enableLegacyMutators`](../deprecated/crud-mutators.md) to `true` in your schema.ts.
2. If you are using the `.client` or `.server` promises with custom mutators, these no longer ever reject. [They now always resolve with a result type that has success/error cases](../mutators.md#waiting-for-results)
3. [Mutators now reject while Zero is offline](../connection.md#offline). You may want to use the new [Connection Status API](../connection.md) to detect this and display custom UI to users.
4. The pattern for reauthentication has changed. The `auth` parameter is now always a `string` – the `function` form is no longer supported. Instead, [call `zero.connection.connect({auth: token})` to reconnect](../auth.md#auth-failure-and-refresh). This was done to simplify the API and support reauth via cookies consistently.
5. The `onError` feature of `Zero` has been removed. It was undocumented and only half-working. Please use the new [Connection Status API](../connection.md) to detect errors and display custom UI to users.
